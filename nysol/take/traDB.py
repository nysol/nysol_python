...
class TraDB(object):
	...
	def addTaxo(self,taxonomy):
		...
		nm.mnjoin(k=self.itemFN,K=itemFN,f=taxoFN,m=tFile).mcut(f=cpara,o=xx1).run()

		
		ipara ="%s,%s"%(self.file,xx1)
		kpara ="%s,%s"%(self.idFN,self.itemFN)
		xx2=tf.file()
		nm.mcat(i=ipara).muniq(k=kpara,o=xx2)
	...
	def repTaxo(self,taxonomy):
		...
		nm.mjoin(k=self.itemFN,K=itemFN,f=taxoFN,m=tFile).mcut(f=cpara).muniq(k=kpara,o=xx1).run()
