...
vector< vector<string> > kgArgs::getparams(const char *cstr, int addCommonArgs) 
{
	vector< vector<string> > rtn(2);
	...
	for(i=_available.begin(); i!=_available.end(); i++){
		if((*i)[0] == '-'){
			rtn[1].push_back((*i).substr(1));
		}
		else{
			rtn[0].push_back((*i).substr(0,(*i).size()-1));
		}
	}
	return rtn;
}
