...
// -----------------------------------------------------------------------------
const char * kgBucket::_ipara[] = {"i",""};
const char * kgBucket::_opara[] = {"o","O",""};

kgBucket::kgBucket(void)
{
	_name    = "kgbucket";
	_version = "###VERSION###";
	_paralist = "f=,i=,o=,k=,O=,n=,F=,-r,-rng,bufcount=,-q";
...
{
    ((kgBucket*)arg)->runErrEnd();
}

int kgBucket::run(int inum,int *i_p,int onum, int* o_p,string &msg)
{
	int sts=1;
