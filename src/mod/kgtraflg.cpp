...
// -----------------------------------------------------------------------------
const char * kgTraflg::_ipara[] = {"i",""};
const char * kgTraflg::_opara[] = {"o",""};

kgTraflg::kgTraflg(void)
{
	_name    = "kgtraflg";
	_version = "###VERSION###";
	_paralist = "f=,i=,o=,a=,delim=,-r";
...
// 入出力ファイルオープン
// -----------------------------------------------------------------------------
void kgTraflg::setArgsMain(void)
{
	_iFile.read_header();
...
void kgTraflg::setArgs(int inum,int *i_p,int onum ,int *o_p)
{
	int iopencnt = 0;
	int oopencnt = 0;
	try{
...
{
    ((kgTraflg*)arg)->runErrEnd();
}


int kgTraflg::run(int inum,int *i_p,int onum, int* o_p,string &msg)
{
	int sts=1;
