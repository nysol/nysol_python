...
char SIMSET_item_ub[20], SIMSET_item_lb[20];   // ignore items of frequency more/less than this, in transaction comparison phase
...
char SIMSET_trsact_ub[20], SIMSET_trsact_lb[20];  // ignore transactions of size more/less than this, in transaction comparison phase
...
double SIMSET_multiply = 0, SIMSET_power = 0, SIMSET_cut = 0.0;
int SIMSET_cores = 1;  // multi-core mode
...
void SIMSET_read_param (int argc, char *argv[]){
  int c=1;
  SIMSET_prog[0] = 0; SIMSET_sep[0] = 0;
...
//  SIMSET_com1 = SIMSET_com;
  ...
  SIMSET_item_lb[0] = SIMSET_item_lb[0] = 0;
  SIMSET_trsact_ub[0] = SIMSET_trsact_lb[0] = 0;
  while ( argv[c][0] == '-' ){
    if ( argc<c+5 ){ SIMSET_error (); return; }
    switch ( argv[c][1] ){
      ...
      break; case 'v': SIMSET_vote_th = atof(argv[c+1]);
      ...
      break; case 'U': sprintf (SIMSET_trsact_ub, " -u %d", atoi(argv[c+1]));
      break; case 'L': sprintf (SIMSET_trsact_lb, " -l %d", atoi(argv[c+1]));
      break; case 'I': sprintf (SIMSET_item_ub, " -U%s %f", argv[c][2]=='I'?"U":"", atof(argv[c+1]));
      break; case 'i': sprintf (SIMSET_item_lb, " -L%s %f", argv[c][2]=='i'?"L":"", atof(argv[c+1]));
    ...
    if ( flag==0 || SIMSET_unify_mark[i] == i ){
      if ( flag == 0 ){
        ...
        FLOOP (j, FF.fstar[i], FF.fstar[i+1]){
          ...
          while (1){
            z++;
            if ( SIMSET_unify_set[e] == e ) break;
            e = SIMSET_unify_set[e];
          }
        }
        if ( z < SIMSET_lb ) continue;
      }
      c = 0;
      FLOOP (j, FF.fstar[i], FF.fstar[i+1]){
        e = FF.edge[j];
        if ( SIMSET_unify_mark[e] != e ) continue;
        while (1){
//if ( fp.buf - fp.buf_org > FILE2_BUFSIZ/2+50 ){ printf ("error buf\n"); }
...
int SIMSET_main (int argc, char *argv[]){
  ...
  char *W = SIMSET_workdir, s1[1024], s2[1024], s3[1024], s4[1024], s5[1024], s6[1024], s7[1024], s8[1024], s9[1024], sk[20], *s10 = " -d 0.001", *f1="__tmp_out0__", *f2="__tmp_out1__", *fn = f1, ff=0;
  ...
  else if ( SIMSET_power != 0.0 ) sprintf (s9, "Z -x %f", SIMSET_power);
  ...
  EXECSUB (SSPC_main, SIMSET_mes, goto END, "sspc 0%s%c%s%s%s%s%s%s%s%s%s%s -M %d \"%s\" %f \"%s__tmp_out__\"",
      SIMSET_prog, SIMSET_com1, SIMSET_rm_dup?"":"1", SIMSET_tpose=='t'?"t":"", s3, s5, s8, sk,
      SIMSET_item_lb, SIMSET_item_ub, SIMSET_trsact_lb, SIMSET_trsact_ub, SIMSET_cores, SIMSET_infname, SIMSET_th1, W);
  unify_flag = 1; SIMSET_nodes = internal_params.l3;

      // convert edge type to list type
  EXECSUB (GRHFIL_main, SIMSET_mes, goto END,"grhfil %sue91%s%s -n " UNIONFIND_IDF " \"%s__tmp_out__\" \"%s__tmp__\"",
  ...
  GRAPH_POLISHING:;
  if ( SIMSET_intgraph ){ 
    EXECSUB (GRHFIL_main, SIMSET_mes, goto END,"grhfil %su \"%s__tmp__\" \"%s__tmp_org__\"",
  ...
  do {  // data polishing loop
    ...
    EXECSUB (SSPC_main, SIMSET_mes, goto END, "sspc 0%s%c%s%s%s%s -l %d -u %d -M %d \"%s__tmp__\" %f \"%s%s\"",
        SIMSET_prog, SIMSET_com, SIMSET_f, s3, s5, s8,
         SIMSET_deg_lb, SIMSET_deg_ub, SIMSET_cores, W, SIMSET_th, W, fn);
    unify_flag = 1; SIMSET_nodes = internal_params.l3;
    siz = internal_params.l1;
    if ( siz == 0 ){ print_err (" no similar pair exists"); break; }

      // check the convergence
    if ( SIMSET_com != 'C' && siz == sizz && siz > 0 ){
...
//        SIMSET_com=='C'?"w":"", SIMSET_com=='C'?"w":"", W, s10, W, W);
      EXECSUB (GRHFIL_main, SIMSET_mes, goto END, "grhfil %sdEe%s -dde%s \"%s__tmp_out0__\"%s \"%s__tmp_out1__\" \"%s__tmp_diff%d__\"", 
#ifdef _REMAIN_DIFFS_
        SIMSET_prog, SIMSET_com=='C'?"w":"", SIMSET_com=='C'?"w":"", W, s10, W, W, count);
#else
        SIMSET_prog, SIMSET_com=='C'?"w":"", SIMSET_com=='C'?"w":"", W, s10, W, W, ff);
#endif
      if ( internal_params.l1 == 0 ) break_flag = 1;  // no difference
      if ( SIMSET_mes ) fprintf (stderr, "#diff edges %lld\n", internal_params.l1);
        // diff of previous & current diffs
...
#else
        EXECSUB (GRHFIL_main, SIMSET_mes, goto END, "grhfil dEe%s -dde%s \"%s__tmp_diff%d__\"%s \"%s__tmp_diff%d__\" \"%s__tmp_diff__\"", 
          SIMSET_com=='C'?"w":"", SIMSET_com=='C'?"w":"", W, (ff+1)%2, s10, W, ff, W);
#endif
        if ( internal_params.l1 == 0 ) break_flag = 1;  // no difference
        if ( SIMSET_mes ) fprintf (stderr, "#diff-diff edges %lld\n", internal_params.l1);
      }
      diff = difff; ff = (ff+1)%2;
    } else diff = -1;
    sizz = siz;

    if ( SIMSET_com == 'C' ){ sprintf (s8, " -W %s__tmp_weight__", W); } // inpro
    EXECSUB (GRHFIL_main, SIMSET_mes, goto END,"grhfil %s%sue91%s%s -n " UNIONFIND_IDF " \"%s%s\" \"%s__tmp__\"",
...
//SIMSET_unify_check (s1, s6, 0);
  ...
  } while (1);    //  repeat at most 5000 times

  if ( SIMSET_com == 'C' ){ // not inpro
    EXECSUB (GRHFIL_main, SIMSET_mes, goto END,"grhfil %sd -1 %f -w \"%s__tmp_weight__\" \"%s__tmp__\" \"%s__tmp_out2__\"",
      SIMSET_prog, SIMSET_cut, W, W, W);
  ...
  EXECSUB (MACE_main, SIMSET_mes, goto END, "mace %sM %s-l %d \"%s__tmp__\" \"%s__tmp_out2__\"",
     SIMSET_prog, SIMSET_sep, SIMSET_th4>0?1:SIMSET_lb, W, W);
  if ( SIMSET_th4 > 0 && SIMSET_unify_mark ){ SIMSET_unify (s2, NULL, 0);}

      // phase4: merge similar cliques
  if ( SIMSET_th2 > 0.0 || SIMSET_th3 > 0.0 ){
    EXECSUB (SSPC_main, SIMSET_mes, goto END, "sspc %s%c %s-M %d \"%s__tmp_out2__\" %f \"%s__tmp2__\"",
       SIMSET_prog, SIMSET_com2, SIMSET_sep, SIMSET_cores, W, SIMSET_th2+SIMSET_th3, W);
  ...
  if ( SIMSET_intersection ){
    EXECSUB (MEDSET_main, SIMSET_mes, goto END, "medset%s %s%s%s-l %d %s\"%s__tmp_out2__\" \"%s\" %f \"%s\"",
        SIMSET_mes2, SIMSET_sep, SIMSET_tpose=='t'?"-t ":"", SIMSET_ratio?"-i ": "", SIMSET_lb,
...
///////////////////////////////////////////////////////////
  END:;
  if ( !SIMSET_leave_tmp_files ) MREMOV (W, "__tmp__", "__tmp2__", "__tmp_out__", "__tmp_out0__", "__tmp_out1__", "__tmp_out2__", "__tmp_weight__", "__tmp_edge__", "__tmp_org__", "__tmp_unify__", "__tmp_diff__", "__tmp_diff0__", "__tmp_diff1__");
