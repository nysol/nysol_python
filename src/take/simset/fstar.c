...
void FSTAR_print (FILE *fp, FSTAR *F){
  ...
  FLOOP (i, 0, F->edge_num+1){
    ...
    fprintf (fp, FSTAR_INTF"%c", y, F->sep);
    if ( F->edge_w ) fprintf (fp, WEIGHTF "%c", F->edge_w[i], F->sep);
  }
  fprintf (fp, "\n");
  END:;
}
...
int FSTAR_eval_edge (FSTAR *F, FSTAR_INT x, FSTAR_INT y, WEIGHT w){
  ...
  if ( F->out_deg && (F->flag&FSTAR_OUT_CHK)){
    if ( !RANGE (F->out_lb, F->out_deg[y], F->out_ub) ) return (0);
    if ( (F->flag&LOAD_BIPARTITE)==0
          && !RANGE(F->out_lb, F->out_deg[x], F->out_ub) ) return (0);
  }
  ...
  if ( ((F->flag & FSTAR_CNT_DEG_ONLY) || F->edge_dir==0) && F->in_deg && (F->flag&FSTAR_DEG_CHK)){
    ...
    } else { if ( !RANGE (F->in_lb, F->in_deg[x], F->in_ub) ) return (0); }
  }
  return (1);
}
...
//  int tpose = F->flag & LOAD_TPOSE, f = F->flag - (F->flag & LOAD_EDGEW);
    ...
    // count #pairs
  C = FILE2_count (fp, (F->flag&(LOAD_ELE+LOAD_TPOSE+LOAD_NUM+LOAD_GRAPHNUM+LOAD_EDGE)) | FILE_COUNT_ROWT | ((F->flag & FSTAR_CNT_IN)? FILE_COUNT_CLMT: 0), 0, 0, 0, (F->flag&LOAD_EDGEW)?1:0, 0);
  ...
  FLOOP (i, 0, C.clms) if ( F->in_deg ) F->in_deg[i] = C.clmt[i];
  mfree (C.rowt, C.clmt);
}
      ...
      if ( !flag || FSTAR_eval_edge (F, x, y, w) ){
        if ( phase == 2 ){
          if ( F->fstar[x+1]>F->fstar[x] && (!(F->flag & LOAD_EDGE) || F->fstar[y+1]>F->fstar[y])){
            ...
            F->edge[F->fstar[x]++] = y;
            if ( (F->flag & LOAD_EDGE) && x != y ){  // undirected && not selfloop
              if ( F->edge_w ) F->edge_w[F->fstar[y]] = w;
              F->edge[F->fstar[y]++] = x;
            }
          }
        } else if (F->flag & LOAD_EDGE){
          F->fstar[x]--;
  ...
  do {
    ...
    if ( F->in_deg || F->out_deg ){
      ...
      x=0; FLOOP (i, 0, F->edge_num){
        ...
        if ( F->edge[i] < F->node_num ) FSTAR_inc_deg (F, x, F->edge[i]);
      }
    }
...
FILE *FSTAR_open_write_file (FSTAR *F, char *fname){
  ...
  if ( F->flag&(LOAD_NUM+LOAD_GRAPHNUM) ){
    fprintf (fp, FSTAR_INTF"%c", F->node_num, F->sep);
    if ( F->flag&LOAD_NUM ) fprintf (fp, FSTAR_INTF"%c", F->node_num, F->sep);
...
void FSTAR_write_graph_ID (FSTAR *F, FILE *fp, FILE *fp2, FSTAR_INT ID){
  ...
  if ( F->flag & FSTAR_INS_ROWID){
    if ( F->flag & LOAD_ELE ){
      fprintf (fp, FSTAR_INTF"%c"FSTAR_INTF, ID, F->sep, ID);
      if ( (F->flag & LOAD_EDGE) && (F->flag & FSTAR_INS_ROWID_WEIGHT) ){ fprintf (fp, "%c1", F->sep); }
      fprintf (fp, "\n");
    } else {
      fprintf (fp, FSTAR_INTF"%c", ID, F->sep);
      if ( fp2 && (F->flag & FSTAR_INS_ROWID_WEIGHT) ) fprintf (fp2, "1%c", F->sep);
      if ( (F->flag & LOAD_EDGEW) && (F->flag & FSTAR_INS_ROWID_WEIGHT)){ fprintf (fp, "1%c", F->sep); }
    }
  }
}
...
int FSTAR_write_graph_item (FSTAR *F, FSTAR_INT x, FSTAR_INT y, WEIGHT w, FILE *fp, FILE *fp2, int *row, FSTAR_INT *prv){
  ...
  if ( !(F->flag&LOAD_RM_DUP) || *row == 0 || *prv != y ){
    if ( F->flag & LOAD_ELE ){
      fprintf (fp, FSTAR_INTF"%c"FSTAR_INTF, x, F->sep, y);
      if (F->flag & LOAD_EDGEW){ fputc (F->sep, fp); fprint_real (fp, w); }
      fprintf (fp, "\n");
    } else {
      fprintf (fp, FSTAR_INTF"%c", y, F->sep);
      if ( fp2 ){ fprint_real (fp2, w); fputc (F->sep, fp2);  }
      if ( F->flag&LOAD_EDGEW ){ fprint_real (fp, w); fputc (F->sep, fp); }
    }
    *row = 1;
  }
