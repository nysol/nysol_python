...
void TRSACT_init (TRSACT *T){
  ...
  T->clm_lb_ = T->clm_ub_ = T->row_lb_ = T->row_ub_ = 0.0;
...
int TRSACT_alloc (TRSACT *T, FILE_COUNT *C){
  ...
  if ( T->row_lb_ ) T->row_lb = T->rows_org * T->row_lb_;
  if ( T->row_ub_ ) T->row_ub = T->rows_org * T->row_ub_;
  if ( T->clm_lb_ ) T->clm_lb = T->clms_org * T->clm_lb_;
  if ( T->clm_ub_ ) T->clm_ub = T->clms_org * T->clm_ub_;
